...
### 可靠性设计（解决 AI 幻觉 / 上下文爆炸 / 不可追溯）

| 问题 | 机制 |
|---|---|
| **AI 幻觉（报不存在的洞）** | ① No PoC = 不存在（无复现证据不算漏洞）② 20 分钟规则（单端点无突破就换攻击面）③ SSTI 双重确认（`{{7*7}}`=49 后再 `{{9*9}}`=81 防巧合）④ 代码层五层门禁 |
| **上下文爆炸** | ① 探针三层懒加载（Tier1 永远跑/Tier2 信号激活/Tier3 反馈，静态站省 ~31KB）② 状态外置 SQLite（上下文不存状态）③ 自动续轮（50% 阈值开新 Session） |
| **不可追溯** | ① 三目录审计（`_work/` 进度 + `_audit_log/` 心跳日志 + `_report/` 证据）② 每个 Agent 写私有文件 + heartbeat ③ 报告带原始 HTTP 证据 |
